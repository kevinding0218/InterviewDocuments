...
- One way to fix things here is to synchronize the access to the array, by adding the synchronized keyword on it like `public void synchronized produce ()` or `public void synchronized consume()` , 
	...
	- So we need a way to "park" a thread while he is waiting for some data to be produced, and when the thread is "parked", it should not block all other threads, so the key held by this thread should be released while this thread is "parked"
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTY3MzkxNDMwNiwtMjI0MjE5NjgzLC00OD
