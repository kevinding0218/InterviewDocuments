...
- gRPC Message Type
	...
	service Employee {
		...
		// Stream of bi-directional: send an employee one at a time and as it saved, server sends the response back down, so client doesn't have to wait for all operations to be complete before it sends the next employee record.
		rpc SaveAll (stream Request) returns (stream Response)
	}
	```
...
``
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE4MDA4Nzk2ODMsLTEwOTAwNTAyMDAsMT
AwMDQ4NDE4OSwxMDMwNDQwODYsLTgyNDQ4NTcxMCwtMTg2NTY4
NzY0OCwtMTM1MDc1NjA4OSwtMTEwNTI5MzE3MiwtMTY1OTg0Nj
c1MywtMTM3NTMwNTIyNiwtMjA0NjM2MTU4NSwtMTgxMzUwODY0
NSwxNDQyNjI5NTk0LC02NTg0MTUwMTAsLTE1MDY3MTMwNTBdfQ
==
-->