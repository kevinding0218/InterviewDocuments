...
- iterate each element in array, adds up to `sum`, update `result` with `Math.max(result, sum - minSum)`, update `minSum` with `Math.min(minSum, sum)`
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwNjgwOTExNjAsNjMyMTU3ODY3LDQ5Nj
